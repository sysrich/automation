...
if command -v jq; then
    ...
    for node in $(echo "$environment" | jq -r '.minions[] | select(.["minion_id"]? == null) | [.addresses.publicIpv4] | join(" ")'); do
        machine_id=$(ssh root@$node -i ../misc-files/id_shared  -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no cat /etc/machine-id)
    ...
    environment=$(echo "$environment" | jq " . + {sshKey: \"`pwd`/../misc-files/id_shared\", sshUser: \"root\"}")
