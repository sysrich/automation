...
if command -v jq; then
    ...
    environment=$(echo "$environment" | jq " . + {dashboardHost: .minions[] | select(.role==\"admin\") | .addresses.publicIpv4, kubernetesHost: .minions[] | select(.role==\"master\") | .addresses.publicIpv4}")
